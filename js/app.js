(() => {
    ...
    const script_form = document.querySelector(".filter");
    if (script_form) {
        const products = document.querySelectorAll(".item-product");
        script_form.addEventListener("change", (() => {
            const checkedInputs = script_form.querySelectorAll('input[type="checkbox"]:checked');
            const selectedValues = Array.from(checkedInputs).map((input => input.value));
            products.forEach((product => {
                const filter = product.dataset.filter;
                if (selectedValues.length === 0 || selectedValues.includes(filter)) product.style.display = ""; else product.style.display = "none";
            }));
        }));
    }
